...
- R11. The `/mcp` route answers on any host; self-hosted needs no host configuration. (Revised post-review:) when `API_HOST` and `APP_HOST` are both set, the API host serves only `/mcp` and the health check — all other paths on it redirect to the app host, so sign-in and app pages never answer on the api subdomain. `API_HOST` also drives displayed URLs (docs page, connect snippets).
...
- **Host handling is opt-in via `API_HOST`.** The `/mcp` route itself answers on any host, so self-hosted needs zero config. (Revised post-review:) setting `API_HOST` + `APP_HOST` activates a routing constraint that restricts the API host to `/mcp` + `/up` and bounces every other path to the app host — the api subdomain never serves sign-in or app pages, which also lets the Cloudflare bot-protection exemption be host-scoped without exposing auth endpoints. No edition branching: the constraint keys purely on configuration.
